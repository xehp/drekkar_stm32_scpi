...
*/
#define DEFAULT_MICRO_VOLTS_PER_UNIT_DC 23000
...
#define DEFAULT_DEVIATION_FACTOR 5
...
#define DEFAULT_MAX_EXT_TEMP_C 95

#define DEFAULT_VOLTAGE_PROBE_FACTOR 500

// See also CMD_MAX_TEMP_HZ in cmd.c.
//#define DEFAULT_MIN_TEMP_HZ 54
#define DEFAULT_MAX_TEMP_HZ 1000

#define DEFAULT_FREQ_AT_20C 66 // Was 78
...
#define DEFAULT_SUPERVICE_DROP_FACTOR 10

#define DEFAULT_LOW_DUTY_CYCLE_PERCENT 45

#define DEFAULT_SUPERVICE_INC_FACTOR 7

#define DEFAULT_MIN_Q_VALUE 23
#define DEFAULT_MAX_Q_VALUE 40

#define DEFAUT_WEB_SERVER_TIMEOUT 0xFFFFFFFF

#define DEFAULT_WALL_TIME 0

#define DEFAULT_SUPERVICE_MARGIN_MV 5000

#define DEFAULT_EXT_MAX_TEMP_DIFF_C 6

// Changes due to voltage need to be slower than that for current
// since the sensor is slower.
#define DEFAULT_maxCurrentStep_ppb 20000
#define DEFAULT_maxVoltageStep_ppb 5000

#define DEFAULT_MAX_LEAK_CURRENT_MA 0

#define DEFAULT_SAVE_COUNTER 0

#define DEFAULT_min_mV_per_mA_ac 0


static const EeDataStruct eeDeault={
	...
	DEFAULT_min_mV_per_mA_ac,
	0, // spare0
	DEFAULT_MIN_Q_VALUE,
	DEFAULT_MAX_Q_VALUE,
	DEFAUT_WEB_SERVER_TIMEOUT,
	DEFAULT_WALL_TIME,
	{0},
	{0},
	DEFAULT_SUPERVICE_MARGIN_MV,
	DEFAULT_EXT_MAX_TEMP_DIFF_C,
	0, // spare
	DEFAULT_maxCurrentStep_ppb,
	DEFAULT_maxVoltageStep_ppb,
	DEFAULT_MAX_LEAK_CURRENT_MA,
	0, // spare2
	0, // spare3
...
static void eepromDataUpgrade(EeDataStruct* e)
{
	if (e->magicNumber == EEDATA_LEGACY_MAGIC_NR)
	{
		...
		e->min_mV_per_mA_ac = DEFAULT_min_mV_per_mA_ac;
		e->maxLeakCurrentStep_mA = DEFAULT_MAX_LEAK_CURRENT_MA;
		e->spare2 = 0;
		e->spare3 = 0;
		e->saveCounter = 0;
...
static int8_t flashSaveBytePacker(EeDataStruct* eedataStruct, uint16_t offset)
{
...
static int8_t flashLoadBytePacker(EeDataStruct* eedataStruct, uint16_t offset)
{
	char fileName[80];
	snprintf(fileName ,sizeof(fileName), "eeprom%d.bin", offset);
...
static void flashWaitAndCheckFlashErrors(int line)
{
	flashWaitIfBusy();
 ...
 * Returns 0 if OK.
 */
static int flashData(const uint64_t* src, const uint32_t nBytes, const uint32_t dstAdr)
{
	ASSERT(nBytes<=2048U);
	ASSERT(nBytes%8 == 0);
...
static int8_t eedataSave(const char* dataPtr, uint16_t dataSize, uint16_t offset)
{
	...
	const int r = flashData((const void*)dataPtr, dataSize, dstAdr);

	logInt3(EEPROM_SAVE_RESULT, r, ee.wallTimeReceived_ms);
...
static int8_t eepromTryLoad(uint16_t offset)
{
	...
	logInt2(EEPROM_LOADED_OK, ee.wallTimeReceived_ms);
...
void eepromLoad(void)
{
	...
	if (r!=0)
	{
		...
		if (r==0)
		{
			logInt2(EEPROM_BACKUP_LOADED_OK, ee.wallTimeReceived_ms);
		}
		else
		{
			eepromSetDefault();
	...
	//debug_print("\n" "microAmpsPerUnit "); debug_print64(ee.microAmpsPerUnitDc);
	//debug_print("\n" "microVoltsPerUnit "); debug_print64(ee.microVoltsPerUnitDc);
	//debug_print("\n" "wantedInternalScanningVoltage_mV "); debug_print64(ee.wantedInternalScanningVoltage_mV);
	debug_print("\n" "wantedCurrent_mA "); debug_print64(ee.wantedCurrent_mA);
	//debug_print("\n" "wantedVoltage_mV "); debug_print64(ee.wantedVoltage_mV);
	//debug_print("\n" "optionsBitMask "); debug_print64(ee.optionsBitMask);
	//debug_print("\n" "superviceDeviationFactor "); debug_print64(ee.superviceDeviationFactor);
	//debug_print("\n" "deviceId "); debug_print64(ee.deviceId);
	//debug_print("\n" "tempMinFreq "); debug_print64(tempMinFreq);
	//debug_print("\n" "tempMaxFreq "); debug_print64(ee.tempMaxFreq);
